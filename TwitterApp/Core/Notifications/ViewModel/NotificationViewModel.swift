...
class NotificationsViewModel: ObservableObject {
    ...
    func fetchNotifications() {
        ...
        NotificationService.shared.fetchNotifications(for: currentUserId) { [weak self] notifications in
    ...
    func sendNotification(toUserId: String, postId: String?, type: NotificationType) {
        guard let currentUserId = Auth.auth().currentUser?.uid else { return }
        NotificationService.shared.sendNotification(toUserId: toUserId, fromUserId: currentUserId, postId: postId, type: type)
    }
